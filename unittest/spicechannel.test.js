...
suite('SpiceChannel', function() {
	...
	suite('#connection', function() {
		...
		test('disconnect should call socketQ disconnect', function() {
			var mock = sinon.mock(socketQ);
			var packetReassemblerMock = sinon.mock(packetReassembler);
			packetReassemblerMock.expects('dispose').once().withExactArgs();
			var expectation = mock.expects('disconnect').once().withExactArgs();
			sut.disconnect();
			expectation.verify();
		});

		test('disconnect should call packet disconnect', function() {
			var mock = sinon.mock(socketQ);
			var packetReassemblerMock = sinon.mock(packetReassembler);
			var expectation = packetReassemblerMock.expects('dispose').once().withExactArgs();
			mock.expects('disconnect').once().withExactArgs();
			sut.disconnect();
			expectation.verify();
		});
	});
